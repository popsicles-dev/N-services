...
from app.tasks.lead_tasks import intent_scoring_task
import uuid
import json
import os
import shutil
...
class IntentScoringRequest(BaseModel):
    """Request model for Intent Scoring."""
    input_filename: str = Field(..., description="Name of audited CSV file to score")
    limit: int = Field(100, ge=1, le=500, description="Number of leads to score")
...
@router.post("/scoring/run", response_model=JobResponse, status_code=status.HTTP_202_ACCEPTED)
async def start_intent_scoring(
    request: IntentScoringRequest,
    db: Session = Depends(get_db)
):
    """Start a background job to score leads using LLM.
    
    Args:
        request: Intent scoring parameters
        db: Database session
    ...
    job = job_service.create_job(
        job_type=JobType.INTENT_SCORING,
        input_params=input_params
    )
    job_id = job.id
    
    # Start Celery task
    intent_scoring_task.delay(
        job_id=job_id,
        input_filename=request.input_filename,
        limit=request.limit
    )
    ...
    return JobResponse(
        job_id=job_id,
        status="pending",
        message="Intent scoring job started"
    )


@router.get("/scoring/job/{job_id}", response_model=JobStatusResponse)
async def get_scoring_status(
    job_id: str,
    db: Session = Depends(get_db)
):
    """Get the status of an intent scoring job."""
    return await get_job_status(job_id, db)


@router.get("/scoring/download/{job_id}")
async def download_scoring_result(
    job_id: str,
    db: Session = Depends(get_db)
):
    """Download the result CSV file for a completed scoring job."""
    return await download_result(job_id, db)
