...
import { useParallaxBackground } from '../hooks/useScrollTimeline'
...
export default function Dashboard() {
    ...
    const scoringSectionRef = useRef(null)
    ...
    const scoringHeaderRef = useScrollAnimation({ triggerOnce: false, rootMargin: '-15% 0px -15% 0px' })
    const scoringCardRef = useScrollAnimation({ triggerOnce: false, rootMargin: '-15% 0px -15% 0px' })
    ...
    const fetchAuditCsvPreview = async (id) => {
        try {
            ...
            // data is an array of rows, first row is headers
            if (data && data.length > 0) {
                ...
                setAuditCsvData({ headers, rows })
                // After successful audit, set up for scoring
                if (auditResults?.result_file) {
                    setScoringFile(auditResults.result_file)
                }
            } else {
                setAuditCsvData({ headers: [], rows: [] })
            }
    ...
    const handleProceedToScoring = () => {
        if (!auditResults?.result_file) return
        setScoringFile(auditResults.result_file)
        scrollToSection(scoringSectionRef)
    }

    const handleStartScoring = async () => {
        if (!scoringFile) return
        ...
        try {
            const data = await ScoringService.runIntentScoring(scoringFile, 100)
            setScoringJobId(data.job_id)
            pollScoringStatus(data.job_id)
        } catch (err) {
            setScoringError('Failed to start scoring: ' + (err.response?.data?.detail || err.message))
            setScoringLoading(false)
        }
    }

    const pollScoringStatus = async (id) => {
        const interval = setInterval(async () => {
            try {
                const job = await ScoringService.getScoringStatus(id)

                if (job.status === 'completed') {
                    clearInterval(interval)
                    setScoringLoading(false)
                    setScoringProgress(100)
                    setScoringResults(job)
                } else if (job.status === 'failed') {
                    clearInterval(interval)
                    setScoringLoading(false)
                    setScoringError('Scoring failed: ' + job.error_message)
                } else {
                    if (job.total_items > 0) {
                        const percent = Math.min((job.processed_items / job.total_items) * 100, 95)
                        setScoringProgress(Math.round(percent))
                    } else {
                        setScoringProgress(prev => Math.min(prev + 2, 95))
                    }
                }
            } catch (err) {
                clearInterval(interval)
                setScoringLoading(false)
                setScoringError('Error checking scoring status')
            }
        }, 3000)
    }
    ...
    return (
        <div className="min-h-screen">
                                        ...
                                        <div className="p-4 border-t border-gray-100 bg-gray-50 flex flex-wrap justify-between items-center gap-3">
                                            <div className="flex gap-3">
                                                ...
                                                <button
                                                    ...
                                                    className="px-6 py-2.5 bg-white border border-gray-200 text-gray-700 text-sm font-semibold rounded-xl hover:bg-gray-50 hover:scale-105 active:scale-95 transition-all"
                                                >
                                                    Start New Audit
                                                </button>
                                            </div>
                                            <button
                                                ...
                                                className="px-6 py-2.5 bg-gradient-to-r from-purple-500 to-violet-500 text-white text-sm font-semibold rounded-xl shadow-lg shadow-purple-500/25 hover:scale-105 active:scale-95 transition-all"
                                            >
                                                Proceed to Intent Scoring →
                                            </button>
                                        </div>
                                    </div>
                                )}
            ...
            <section id="intent-scoring" ref={scoringSectionRef} className="py-20 px-6 bg-gradient-to-b from-white to-violet-50 scroll-mt-24">
                <div className="max-w-7xl mx-auto">
                    {/* Section Header */}
                    <div ref={scoringHeaderRef} className="fade-in-view text-center mb-12">
                        ...
                        <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">Intent Scoring</h2>
                        <p className="text-gray-500 max-w-xl mx-auto">Use AI to analyze and score your leads based on their likelihood to convert.</p>
                    </div>

                    {/* Scoring Card */}
                    <div ref={scoringCardRef} className="fade-in-view bg-white rounded-3xl shadow-xl border border-gray-100 p-8 hover-lift">
                        {!scoringFile ? (
                            <div className="space-y-8">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {/* Upload File */}
                                    <div className="border-2 border-dashed border-gray-200 rounded-2xl p-8 text-center hover:border-violet-400 transition-colors">
                                        <div className="w-16 h-16 bg-violet-50 rounded-full flex items-center justify-center mx-auto mb-4">
                                            <svg className="w-8 h-8 text-violet-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path>
                                            </svg>
                                        </div>
                                        <h3 className="text-lg font-semibold text-gray-900 mb-2">Upload Audited CSV</h3>
                                        <p className="text-sm text-gray-500 mb-4">Upload a CSV that has already been SEO audited</p>
                                        <input
                                            type="file"
                                            accept=".csv"
                                            onChange={async (e) => {
                                                ...
                                                try {
                                                    const uploadData = await AuditService.uploadFile(file)
                                                    setScoringFile(uploadData.filename)
                                                } catch (err) {
                                                    setScoringError('Failed to upload file: ' + (err.response?.data?.detail || err.message))
                                                }
                                            }}
                                            className="hidden"
                                            id="scoring-upload"
                                        />
                                        <label htmlFor="scoring-upload" className="inline-block px-6 py-2.5 bg-violet-600 text-white text-sm font-medium rounded-xl hover:bg-violet-700 transition-colors cursor-pointer">
                                            Choose File
                                        </label>
                                    </div>
                                    ...
                                    <div className="border-2 border-gray-200 rounded-2xl p-8 text-center hover:border-green-400 transition-colors">
                                        ...
                                        <h3 className="text-lg font-semibold text-gray-900 mb-2">From SEO Audit</h3>
                                        <p className="text-sm text-gray-500 mb-4">Use the "Proceed to Intent Scoring" button after audit</p>
                                        <button onClick={() => scrollToSection(auditSectionRef)} className="px-6 py-2.5 bg-green-600 text-white text-sm font-medium rounded-xl hover:bg-green-700 transition-colors">
                                            Go to SEO Audit
                                        </button>
                                    </div>
                                </div>
                                ...
                                {scoringError && (
                                    <div className="p-4 bg-red-50 text-red-600 rounded-xl border border-red-100">{scoringError}</div>
                                )}
                                ...
                                <div className="bg-violet-50 border border-violet-100 rounded-xl p-4">
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="text-sm text-violet-700">
                                            <p className="font-semibold mb-1">How Intent Scoring Works</p>
                                            <ul className="list-disc list-inside space-y-1 text-violet-600">
                                                <li><strong>High Potential (8-10):</strong> Missing title, H1, or generic content</li>
                                                <li><strong>Medium (5-7):</strong> Minor structural issues</li>
                                                <li><strong>Low (1-4):</strong> All elements present and customized</li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <>
                                <div className="flex items-center justify-between mb-6">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 bg-violet-50 rounded-xl flex items-center justify-center">
                                            <svg className="w-6 h-6 text-violet-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                        </div>
                                        <div>
                                            <p className="font-semibold text-gray-900">Ready for Scoring</p>
                                            <p className="text-sm text-gray-500 truncate max-w-md">{scoringFile}</p>
                                        </div>
                                    </div>

                                    <button
                                        onClick={handleStartScoring}
                                        disabled={scoringLoading || scoringResults}
                                        className={`px-6 py-3 bg-gradient-to-r from-violet-500 to-purple-500 text-white font-semibold rounded-xl shadow-lg shadow-violet-500/25 transition-all ${(scoringLoading || scoringResults) ? 'opacity-60 cursor-not-allowed' : 'hover:scale-105 active:scale-95'}`}
                                    >
                                        {scoringLoading ? 'Scoring...' : scoringResults ? '✓ Scored' : 'Start Intent Scoring'}
                                    </button>
                                </div>
                                ...
                                {scoringLoading && (
                                    <div className="mb-6">
                                        <div className="flex justify-between items-center mb-2">
                                            <p className="text-sm font-medium text-gray-700">Analyzing leads with AI...</p>
                                            <p className="text-sm font-bold text-violet-600">{scoringProgress}%</p>
                                        </div>
                                        <div className="w-full bg-gray-100 rounded-full h-3">
                                            <div className="bg-gradient-to-r from-violet-500 to-purple-500 h-3 rounded-full transition-all duration-500" style={{ width: `${scoringProgress}%` }}></div>
                                        </div>
                                        <p className="text-xs text-gray-400 mt-2">⏱️ AI is evaluating each lead's intent signals...</p>
                                    </div>
                                )}
                                    ...
                                    <div className="border border-gray-100 rounded-2xl overflow-hidden">
                                        <div className="p-6 bg-gradient-to-r from-violet-50 to-purple-50">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <h4 className="font-semibold text-gray-900 mb-1">Intent Scoring Complete!</h4>
                                                    <p className="text-sm text-gray-500">Processed {scoringResults.processed_items} of {scoringResults.total_items} leads</p>
                                                </div>
                                                <div className="flex gap-3">
                                                    <button
                                                        onClick={() => window.open(ScoringService.getDownloadUrl(scoringJobId), '_blank')}
                                                        className="px-6 py-2.5 bg-gradient-to-r from-violet-500 to-purple-500 text-white text-sm font-semibold rounded-xl shadow-lg shadow-violet-500/25 hover:scale-105 active:scale-95 transition-all"
                                                    >
